...
export function OrderCard({ order, requestStatus, onDelete, onRequestJoin, onWithdraw }: OrderCardProps) {
  ...
  const hostelLabel = order.hostel
    .replace(/^HB4\s+/i, "")
    .replace(/-WING/i, "-Wing")
    .replace(/\s*\(([^)]+)\)$/, " · $1");
  ...
  return (
    <div className={`order-card ${isOwn ? "order-card--own" : ""} ${requestStatus === "declined" ? "order-card--declined" : ""}`}>
      ...
      <div className="order-card__header">
        ...
        <span className={`countdown-badge${countdown.state === "urgent" ? " countdown-badge--urgent" : countdown.state === "amber" ? " countdown-badge--amber" : ""}`} aria-label={`Closes in ${countdown.label}`}>
          <span className="countdown-badge__label">Closes in</span> {countdown.label}
        </span>
      </div>

      {/* Public summary. Exact pickup details stay private. */}
      ...
      <div className="order-card__actions">
        {isOwn ? (
          ...
          <a
            ...
            onClick={() => trackEvent("whatsapp_clicked", { hostel: order.hostel, platform: order.platform })}
          >
